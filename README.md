...
This creates 15 contacts with randomly generated tasks to demonstrate pagination and filtering.
...
List contacts (supports `?search=`, `?status=`, `?ordering=`, pagination)
...
Contact detail (includes `open_tasks_count`)
...
List tasks (supports `?contact_id=`, `?is_done=`, `?priority=`, `?due_from=`, `?due_to=`)
